...
// context donut: VChart pie (Echo footer dialect), lives inside the footer panel
{
  ...
  check('context chart: numeric value + smart disp label', JSON.stringify(chart?.chart_spec?.data?.values) === JSON.stringify([{ type: '已用', value: 32000, disp: '32K' }, { type: '剩余', value: 96000, disp: '96K' }]) && chart.chart_spec.label?.formatter === '{disp}')
