...
export function apply(ctx, config) {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (event.type === 'turn/start') {
      ...
      if (!liveTurns.has(sid)) {
        const st = { cardMessageId: '', text: '', texts: [], reasoning: '', tools: [], chunked: false, phase: 'thinking', frame: 0, thinkSecs: null, startedAt: Date.now(), dirty: false, timer: null, model: '' }
    ...
    if (event.type === 'turn/end') {
      ...
      if (st.text.trim() || st.reasoning.trim() || st.tools.length) {
        const card = buildDoneCard(finalText, { elapsed, model: st.model, reasoning: st.reasoning, thinkSecs: st.thinkSecs ?? undefined, tools: st.tools })
        if (st.cardMessageId) {
          void feishu.updateCard(st.cardMessageId, card).catch(() => {})
        } else {
          void feishu.sendCard(chatId, card).catch(() => {})
   ...
   * (freeze what it has, mark it continued) and start a fresh streaming card
   * for the rest of the turn so nothing ever appends behind the approval card.
   */
  function sealAndSplitCard(sid) {
    const st = liveTurns.get(sid)
    if (!st) return
    const chatId = sessions.chatOf(sid)
    if (!chatId) return
    ...
    // seal current card as a partial/continued done card
    if (st.cardMessageId) {
    ...
    // reset accumulator state; a fresh card will be created on the next token
    st.cardMessageId = ''
    st.text = ''
    st.texts = []
    st.reasoning = ''
    ...
    st.phase = 'thinking'
    st.dirty = false
    // stop the old timer; a new one starts when the new card is created (turn/start guard
    // skips re-init because liveTurns still holds st, so create the card right here)
    stopCardTimer(sid)
    feishu.sendCard(chatId, buildThinkingCard({ frame: 0 }))
      .then((mid) => {
        st.cardMessageId = mid
  ...
  async function handleInbound(msg) {
    ...
    if (approvals && (text === '允许' || text === '拒绝' || text.toLowerCase() === 'allow' || text.toLowerCase() === 'deny')) {
      ...
      if (pending) {
        ...
        await feishu.sendText(msg.chatId, res.text || '已处理', msg.messageId).catch(() => {})
        return
      }
  ...
  ctx.effect(() => {
    ...
    if (config.autoStart && config.appId && config.appSecret) {
        ...
        onCardAction: async (evt) => {
          try {
            ...
            if (evt.action?.value?.bridge_approval) {
              ...
              const outcome = evt.action.value.outcome === 'allowed-once' ? 'allowed-once' : 'rejected'
      ...
      approvals.onApprovalCardSent = sealAndSplitCard
      approvals.start()
    } else {
