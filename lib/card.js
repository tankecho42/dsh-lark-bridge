...
export function buildContextDonutChart(contextPressure) {
  ...
  // values stay raw tokens; VChart formats axis labels itself and smartTokens
  // keeps each slice small enough to label — no forced K unit on everything
  return {
    tag: 'chart',
    chart_spec: {
      type: 'pie',
      ...
      color: ['#3370ff', '#dee0e3'],
      data: { values: [
        { type: '已用', value: context.chartUsedTokens, disp: smartTokens(context.chartUsedTokens) },
        { type: '剩余', value: Math.max(1, context.remainingTokens), disp: smartTokens(context.remainingTokens) },
      ]},
      valueField: 'value', categoryField: 'type',
      label: { visible: true, formatter: '{disp}' },
      legends: { visible: true, orient: 'bottom' },
      title: { text: '上下文占用（tokens）' },
    },
  }
}
...
export function buildTokenBarChart(tokenUsage) {
  ...
  // Echo semantics: full prompt = cached + fresh + write; cache bar is the cached slice of it
  ...
  // raw token counts — smart units only kick in via axis formatting; labels
  // stay exact (16,384 reads better than 16.4K when you're checking numbers)
  return {
    tag: 'chart',
    chart_spec: {
      type: 'bar',
      color: ['#3370ff'],
      data: { values: [
        { name: '输入', value: input, disp: smartTokens(input) },
        { name: '输出', value: output, disp: smartTokens(output) },
        { name: '缓存命中', value: cache, disp: smartTokens(cache) },
      ]},
      xField: 'name', yField: 'value',
      label: { visible: true, formatter: '{disp}' },
      title: { text: '输入 / 输出 / 缓存命中（tokens）' },
    },
  }
}

/** Smart token units: raw number under 1K, K above, M above 1M. */
function smartTokens(n) {
...
if (parts.length) {
    ...
    if (context) {
      inner.push({ tag: 'markdown', content: `**上下文占用 ${context.percent}%** · ${smartTokens(context.usedTokens)} / ${smartTokens(context.contextWindow)} tokens` })
      const donut = buildContextDonutChart(contextPressure)
      if (donut) inner.push(donut)
    }
    if (tokenUsage) {
      inner.push({ tag: 'markdown', content: `**本轮 token** · in ${smartTokens(inputT + cacheT)} · out ${smartTokens(tokenUsage.outputTokens || 0)} · 缓存命中 ${cachePct ? cachePct + '%' : '—'}` })
