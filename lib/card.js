  ...
  return {
    ...
    chart_spec: {
      ...
      data: [{
        type: 'pie',
        hole: 0.68,
        values: [context.chartUsedTokens, context.remainingTokens],
        labels: ['已用', '剩余'],
        textinfo: 'label+percent',
        hoverinfo: 'label+value+percent',
        sort: false,
        direction: 'clockwise',
        marker: { colors: ['#3370ff', '#dee0e3'] },
      }],
      layout: {
        type: 'base',
        title: { text: `Context ${context.percent}%`, font: { size: 14 } },
        showlegend: false,
...
export function buildDoneCard(text, { elapsed, model, reasoning = '', thinkSecs, tools = [], tokenUsage, contextPressure, continued = false } = {}) {
  ...
  const detailRows = [
    { 项目: '模型', 内容: model || '—' },
    { 项目: 'Context', 内容: contextSummary(contextPressure) },
    { 项目: 'Token', 内容: tokenSummary(tokenUsage) },
    { 项目: '工具调用', 内容: `${tools.length} 次${tools.length ? '（见上方各折叠栏）' : ''}` },
    { 项目: '整体时长', 内容: elapsed != null ? fmtSmartSec(elapsed) : '—' },
    ...(thinkSecs != null ? [{ 项目: '思考', 内容: fmtSmartSec(thinkSecs) }] : []),
    ...(reasoning.trim() ? [{ 项目: '思考内容', 内容: `${reasoning.trim().length} 字符（见上方折叠栏）` }] : []),
    ...(continued ? [{ 项目: '分段', 内容: '本卡为前半段，后续内容见下一条' }] : []),
  ]
  // Keep the body markdown-only: a nested table makes Feishu discard the panel.
  const footerBody = [{ tag: 'markdown', content: detailRows.map((r) => `- **${r['项目']}**：${r['内容']}`).join('\n') }]
  const contextChart = buildContextDonutChart(contextPressure)
  if (contextChart && FOOTER_CHART_INSIDE_PANEL) footerBody.push(contextChart)
  elements.push({ tag: 'hr' })
  if (contextChart && !FOOTER_CHART_INSIDE_PANEL) elements.push(contextChart)
  elements.push({
    tag: 'collapsible_panel',
    expanded: false,
