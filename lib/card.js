...
export function buildContextDonutChart(contextPressure) {
  ...
  return {
    ...
    chart_spec: {
      ...
      color: ['#3370ff', '#dee0e3'],
      data: { values: [
        { type: '已用', value: smartTokens(context.chartUsedTokens) },
        { type: '剩余', value: smartTokens(context.remainingTokens) },
      ]},
...
export function buildTokenBarChart(tokenUsage) {
  ...
  return {
    ...
    chart_spec: {
      ...
      color: ['#3370ff'],
      data: { values: [
        { name: '输入', value: smartTokens(input) },
        { name: '输出', value: smartTokens(output) },
        { name: '缓存命中', value: smartTokens(cache) },
      ]},
