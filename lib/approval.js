...
export class ApprovalBridge {
  ...
  constructor({ webUrl, feishu, sessions, log }) {
    ...
    this.onApprovalResolved = null // hook: (sessionId, outcome) → start continuation card when allowed
    this.started = false
    this.abort = null
  }
  ...
  async _onApprovalRequested(envelope) {
    ...
    if (this.autoAllow.has(`${p.sessionId}\u0000${entry.toolName}`)) {
      this._respond(entry, 'allowed-once').then((ok) => {
        if (ok && this.onApprovalResolved) {
          try { this.onApprovalResolved(String(p.sessionId), 'allowed-once') } catch { /* best effort */ }
        }
      }).catch(() => {})
        ...
        columns: [
          {
            ...
            elements: [{ tag: 'button', text: { tag: 'plain_text', content: '✅ 允许一次' }, size: 'medium', type: 'primary', value: { bridge_approval: rpcId, outcome: 'allowed-once' } }],
          },
          {
            tag: 'column', width: 'weighted', weight: 1,
            elements: [{ tag: 'button', text: { tag: 'plain_text', content: '🔓 允许本会话' }, size: 'medium', type: 'primary', value: { bridge_approval: rpcId, outcome: 'allowed-session' } }],
          },
          {
            tag: 'column', width: 'weighted', weight: 1,
            elements: [{ tag: 'button', text: { tag: 'plain_text', content: '❌ 拒绝' }, size: 'medium', type: 'danger', value: { bridge_approval: rpcId, outcome: 'rejected' } }],
          },
        ],
      },
      md('*按钮无反应时，直接回复「允许」或「拒绝」也可以；「允许本会话」= 本会话内该工具不再询问*'),
    ], { template: 'orange' })
  ...
  async resolve(rpcId, outcome) {
    ...
    if (outcome === 'allowed-session') {
      // remember the grant, then answer this request as a normal allow
      this.autoAllow.add(`${entry.sessionId}\u0000${entry.toolName}`)
    ...
    if (ok && this.onApprovalResolved) {
      try { this.onApprovalResolved(String(entry.sessionId), outcome) } catch (err) {
        this.log('approval continue failed', { err: String(err?.message || err) })
      }
    }
    if (entry.cardMessageId) {
      const label = outcome === 'allowed-once' && this.autoAllow.has(`${entry.sessionId}\u0000${entry.toolName}`)
        ? '✅ 已允许（本会话内该工具不再询问）'
        : outcome === 'allowed-once' ? '✅ 已允许' : '❌ 已拒绝'
      void this.feishu.updateCard(entry.cardMessageId, cmdCard(`🔐 权限确认 ${label === '✅ 已允许（本会话内该工具不再询问）' ? '✅ 已允许+记住' : label}`, [
        md(`工具 **${entry.toolName}** 的请求已${outcome === 'allowed-once' ? '允许' : '拒绝'}${label.includes('不再询问') ? '，本会话内该工具的后续请求将自动放行' : ''}，任务继续。`),
