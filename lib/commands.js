...
registerCommand({
  ...
  async run({ ctx, sessions, feishu, chatId }) {
    ...
    if (diagnostics) {
      ...
      add(diagnostics.apiFailures ? 'warn' : 'ok', 'API 失败', `${diagnostics.apiFailures || 0} 次；资源下载 ${diagnostics.resourceDownloads || 0} 个`)
      if (diagnostics.bridge) {
        if (diagnostics.bridge.plugin && diagnostics.bridge.version) {
          add('ok', '插件版本', `${diagnostics.bridge.plugin}@${diagnostics.bridge.version}`)
        }
        add(diagnostics.bridge.healthFailures ? 'warn' : 'ok', '健康监控', `连续失败 ${diagnostics.bridge.healthFailures || 0}；告警 ${diagnostics.bridge.healthAlerts || 0}`)
        add('ok', '健康端点', `${diagnostics.bridge.healthHost}:${diagnostics.bridge.healthPort || '(启动中)'} /healthz /readyz /metrics`)
      }
      if (diagnostics.media) {
        add('ok', '附件策略', `图片≤${formatBytes(diagnostics.media.maxImageBytes)}；文件≤${formatBytes(diagnostics.media.maxFileBytes)}；保留 ${diagnostics.media.retentionDays} 天`)
      }
    }

    add('warn', '模型 fallback', '当前 DSH AgentOptions 尚无正式 fallback chain 字段；桥不会伪造宿主能力')
