  ...
  async run({ ctx, feishu, chatId, arg }) {
    ...
    if (discovered.length) {
      const rows = discovered.map((d) => [
        d.provider === sel?.provider && d.model === sel?.model ? '✅ 当前' : '',
        d.model,
        d.provider,
      ])
...
// ---------------------------------------------------------------------------
// /tools — read-only inventory (tools service view + plugin inventory)
// ---------------------------------------------------------------------------
registerCommand({
  name: 'tools',
  usage: '/tools',
  ...
  async run({ ctx, feishu, chatId }) {
    const rows = []
    for (const t of visibleTools(ctx)) rows.push([t, '工具'])
    try {
      const inv = ctx.get('pluginInventory')
      if (inv?.list) {
        const plugins = (await inv.list()) || []
        for (const p of plugins) rows.push([String(p.name || p.id), p.disabled ? '插件（停用）' : '插件'])
      }
    } catch { /* optional */ }
