...
registerCommand({
  ...
  async run({ ctx, feishu, chatId, arg }) {
    ...
    await feishu.sendCard(chatId, cmdCard('🤖 模型配置', elements))
  },
})
...
async function applyModelSwitch({ ctx, feishu, chatId, provider, model }) {
  ...
  try {
    ...
    await feishu.sendText(chatId, `⚠️ 保存失败：${err?.message || err}`)
  }
}
...
registerCommand({
  ...
  async run({ ctx, sessions, feishu, chatId }) {
    ...
    if (selection?.provider && selection?.model) add('ok', '默认模型', `${selection.provider} / ${selection.model}`)
