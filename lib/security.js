...
export class AccessPolicy {
  constructor({ allowedUserIds = [], allowedChatIds = [], adminUserIds = [], warn = () => {} } = {}) {
  ...
  canApprove(userId, requesterId, context = {}) {
    if (!this.allowsUser(userId)) return false
    if (this.isAdmin(userId)) return true
    if (!requesterId) {
      // Preserve the legacy fallback for compatibility, but make every use auditable.
      this.warn('approval requester missing; legacy fallback allowed', {
        ...context,
        userId: String(userId || ''),
      })
      return true
    }
    return String(userId || '') === String(requesterId)
  }
}
